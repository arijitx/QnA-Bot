...
class BertForQuestionAnswering(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.yes_output = nn.Linear(config.hidden_size,1)
        self.no_output = nn.Linear(config.hidden_size,1)
        self.unknow_ouput = nn.Linear(config.hidden_size,1)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None):
        ...
        yes_logits = self.yes_output(sequence_output[:,0,:])
        ...
        if start_positions is not None and end_positions is not None:
            ...
            targets = []
            span_idx = int(logits.size(1)*logits.size(1))
            # print(start_positions,end_positions)
            for i in range(start_positions.size(0)):
                if start_positions[i] == -1 and end_positions[i] == -1:
                    targets.append(span_idx + 2)

                if start_positions[i] == -1 and end_positions[i] == 0:
                    # code for yes
                    targets.append(span_idx)

                if start_positions[i] == 0 and end_positions[i] == -1:
                    # code for no 
                    targets.append(span_idx+1)

                if start_positions[i] != -1 and end_positions[i] != -1:
                    if start_positions[i] == ignored_index or end_positions[i] == ignored_index:
                        targets.append(ignored_index*ignored_index+3)
                        rejected += 1
                    else:
                        targets.append(start_positions[i]*logits.size(1) + end_positions[i])

            targets = torch.LongTensor(targets).cuda()
            # print(targets)
            # targets = torch.LongTensor(targets)
            loss_fct = CrossEntropyLoss(ignore_index=ignored_index*ignored_index+3)

            total_loss = loss_fct(ex_score,targets)

            return total_loss,rejected
        else:
            return ex_score
